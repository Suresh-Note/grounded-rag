...
**Run the full stack**
```bash
git clone https://github.com/Suresh-Note/grounded-rag.git
cd grounded-rag
...
> **Is this deployable?**
>
> Yes. `docker-compose.yml` orchestrates five services: `qdrant`, `redis`, `api`, `celery_worker`, and `ui` (the React/Vite frontend in `aegis-ui/`). The LLM itself runs outside the container network via Ollama on the host. Deploys to AWS ECS, GCP Cloud Run, or on-premise infrastructure.
...
│   └── src/App.tsx              # Master-detail findings dashboard, live SSE progress
└── Agentic-RAG-for-Enterprise-Compliance/    # FastAPI + Celery backend
    ├── src/
    │   ├── agents/
